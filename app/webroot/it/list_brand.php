...
$b_type = $_POST['b_type'] ? $_POST['b_type'] : $_GET['b_type'];
$b_type = $b_type == '' ? 0 : $b_type;

// to display the data using status filter
if(isset($_POST['b_status'])){
	$b_status = $_POST['b_status'];
}else if(isset($_GET['b_status'])){
...
if($_POST){
	...
	$post_url .= '&b_type='.$b_type;
	$post_url .= '&b_status='.$b_status;
}
...
$query = "CALL it_list_brand('".$keyword."','".$b_type."','".$b_status."','0','0','','','".$_GET['action']."')";

try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing count brand page');
	}
	...
	if($count == 0){
		if($keyword){
			$alert_msg = 'No brand "' .$keyword. '" is found in our database';
		}else{
			$alert_msg = 'No brand details found in our database';
		}
	}
   ...
   include('paging_call.php');	
	...
	$sort_fields = array('1' => 'b_name','b_type','created_date','modified');
	$org_fields = array('1' => 'title', 'type','created_date','modified_date');
...
// fetch all records
$query =  "CALL it_list_brand('".$keyword."','".$b_type."','".$b_status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";

try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list brand page');
	}
	...
	if(($_GET['action'] == 'export') && ($count > 0)){ 
      ...
      $excelObj->printHeader($header = array('Title','Type','Created Date','Status') ,$col = array('A','B','C','D'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('title','type','created_date','status'),'Brands_'.$current_date); 				
	}
		
	// assign login status into array 
	$type = array('' => 'All Status', '1' => 'Active', '0' => 'Inactive');
	$b_type_data = array('' => 'Brand Type', 'S' => 'Software Type', 'H' => 'Hardware Type');

	// create,update,delete message validation
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
 	 $success_msg = 'Brand details ' . $_GET['status'] . ' successfully';
	}else if($_GET['status'] == 'not_deleted'){
		$erro_msg = "You cannot delete unless you remove this brand from ".$_GET['type'];	
	}
...
$smarty->assign('b_status', $b_status);
$smarty->assign('b_type', $b_type);
$smarty->assign('b_type_data', $b_type_data);
...
// assign page title
$smarty->assign('page_title' , 'Brand - IT');  
// assigning active class status to smarty menu.tpl
$smarty->assign('settings_active' , 'active'); 	  
// display smarty file
$smarty->display('list_brand.tpl');
?>