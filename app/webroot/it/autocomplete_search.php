...
}else if($_GET['page'] == 'list_billing_hardware'){
	// get matched data from billing hardware
	$query = "CALL it_search_billing_hardware('".$keyword."')";
	try{	
		if(!$result = $mysql->execute_query($query)){
			throw new Exception('Problem in executing billing hardware page');
		}
		// iterate until get the matched results
		while($obj = $mysql->display_result($result)){
			$data[] = strtolower($fun->match_results($keyword,$obj['title']));		
			$data[] = strtolower($fun->match_results($keyword,$obj['inventory_no']));
			$data[] = strtolower($fun->match_results($keyword,$obj['vendor_city']));
			$data[] = strtolower($fun->match_results($keyword,$obj['vendor_company']));
		}
...
}else if($_GET['page'] == 'list_ticket_type'){
	// get matched data from ticket type
	$query = "CALL it_search_ticket_type('".$keyword."')";
	try{	
		if(!$result = $mysql->execute_query($query)){
			throw new Exception('Problem in executing ticket type page');
		}
		// iterate until get the matched results
		while($obj = $mysql->display_result($result)){
