	...
	<div class="container-fluid" id="content">
		<div id="main">
            ...
            <a href="add_assign_asset.php"><button type="button" val="add_assign_asset.php" class="jsRedirect btn btn-primary" style="float:right"><i class="icon-plus"></i> Assign Asset</button></a>
				{if !$ALERT_MSG}            
            	<a href="list_assign_asset.php?action=S&keyword={$smarty.post.keyword}&emp_name={$smarty.post.emp_name}&f_date={$f_date}&t_date={$t_date}"><button type="button" val="list_assign_asset.php?action=H&keyword={$smarty.post.keyword}&emp_name={$smarty.post.emp_name}&f_date={$f_date}&t_date={$t_date}" class="jsRedirect btn btn-primary" style="float:right;margin-right:20px;"><i class="icon-reply"></i> Export Assigned Software </button></a>		           
            	<a href="list_assign_asset.php?action=H&keyword={$smarty.post.keyword}&emp_name={$smarty.post.emp_name}&f_date={$f_date}&t_date={$t_date}"><button type="button" val="list_assign_asset.php?action=S&keyword={$smarty.post.keyword}&emp_name={$smarty.post.emp_name}&f_date={$f_date}&t_date={$t_date}" class="jsRedirect btn btn-primary" style="float:right;margin-right:20px;"><i class="icon-reply"></i> Export Assigned Hardware </button></a>		
				{/if} 				
            </div>			
