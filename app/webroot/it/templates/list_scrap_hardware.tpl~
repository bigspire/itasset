			...
			<div class="container-fluid">
				...
				<div class="breadcrumbs">
					<ul>
						...
						<li>
							<a href="list_scrap_hardware.php">List Scrap Hardware</a>
						</li>
					</ul>
				</div>
				{if $ALERT_MSG}
				...
				<div class="row-fluid  footer_div previewDiv" >
					<div class="span12">
						...
						<div class="box-title">
								<h3><i class="icon-list"></i>Scrap Hardware</h3>
						</div>
				<form action="list_scrap_hardware.php" name="" id="formID" class="" method="post" accept-charset="utf-8">							
             ...
             {if !$ALERT_MSG} 
             <a href="list_scrap_hardware.php?action=export&keyword={$smarty.post.keyword}&hw_type={$smarty.post.hw_type}&f_date={$smarty.post.f_date}&t_date={$smarty.post.t_date}"><button type="button" val="list_scrap_hardware.php?action=export&keyword={$smarty.post.keyword}&hw_type={$smarty.post.hw_type}&f_date={$smarty.post.f_date}&t_date={$smarty.post.t_date}" class="jsRedirect btn btn-primary" style="float:right;margin-right:20px;"><i class="icon-reply"></i> Export</button></a>			
            {/if}
            </div>			
				<table class="table table-hover table-nomargin table-bordered usertable dataTable">
					<thead>
						<tr>	
							<th width="200">
								<a href="list_scrap_hardware.php?field=type&order={$order}&page={$smarty.get.page}&keyword={$keyword}&hw_type={$hw_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_type}">Type</a></th>
							<th width="200">
								<a href="list_scrap_hardware.php?field=brand&order={$order}&page={$smarty.get.page}&keyword={$keyword}&hw_type={$hw_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_brand}">Brand</a></th>		
							<th width="200">
								<a href="list_scrap_hardware.php?field=model_id&order={$order}&page={$smarty.get.page}&keyword={$keyword}&hw_type={$hw_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_model_id}">Model Id</a></th>
							<th width="200">
								<a href="list_scrap_hardware.php?field=inventory_no&order={$order}&page={$smarty.get.page}&keyword={$keyword}&hw_type={$hw_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_inventory_no}">Inventory No</a></th>
							<th width="200">
								<a href="list_scrap_hardware.php?field=location&order={$order}&page={$smarty.get.page}&keyword={$keyword}&hw_type={$hw_type}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_location}">Location</a></th>
							<th width="200">
				...
				<input type="hidden" id="page" value="scrap_hardware">
			</form>						
		 </div>
		</div>
		</div>
