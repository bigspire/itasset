...
// get database values
$query = "CALL it_view_ticket_message('$id')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in geting ticket messages');
	}
  	// calling mysql fetch_result function
	$i = '0';
	...
	{
		$data1[] = $row;
		$data1[$i]['created_date'] = $fun->date_time($row['created_date']);
		$data1[$i]['status'] = $fun->ticket_status($row['status']);
		$i++;
	}
	// free the memory
	$mysql->clear_result($result);
