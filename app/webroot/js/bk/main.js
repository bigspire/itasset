$(document).ready(function() { 	
	...
	if($('#new_f').length > 0){	
		window.setTimeout(new_flash, 3000);
	}
	
	/* function to scroll org tree */
	window.onload = function() {
		//window.setTimeout(scroll_org, 2000);
	};
	...
	$('.shareReply').on('keydown',function(e){ 
		...
		if(keyCode == 13){ 					
			...
			// if validation success
			if(valid){
				update_reply_share(value,id,userid);			
			}
		}
	});
   ...
   /* allow only numeric */
	...
	$("#expForm").delegate(".amtVal", "keyup keypress keydown blur change", function (event) {
		...
		$('.save_journey').click(function(){			
			...
			var submit = true;	var submit2 = true;		
			// for return trip
	...
	*/
	...
	$(".notify").click(function(){
		...
		$('#recent-tab > li.dashList').click(function (){
			...
			if($(this).find('.radius5').is(':visible') == true){
				if(tab == 'chat' || tab == 'news' || tab == 'poll' || tab == 'gal'){				
	...
	function load_share_data(track_load){
		...
		$('#share_results').load(root+'home/load_share/', {'page':track_load}, function(response) {	
		track_load++;
		$('#share_loading').hide();
		$('#shareScroll').scrollTo(0);
		}); //load first group
	...
	function update_reply_share(value, id,userid){  
		$('#busy-indicator').show();		
		$.ajax({
			 url: $('#share_reply_url').val()+'?tsk='+$.trim(value)+'&id='+id+'&userid='+userid			  
			}).done(function( html ) { 
				$('#busy-indicator').hide(); 
	...
	function load_page(valid,dataElem,url){ 
		...
		$('#busy-indicator').show();
		 ...
		 jQuery.ajax({
                url: $('#'+url).val()+'?tsk='+new_value+hdn_id,
                type: "POST",
                contentType: false,
                processData: false,
                data: function() {
				...
				if(full_page == 1){
					...
					if(response[1] != ''){ 
						...
						if(responseId != 'plan' && responseId != ''){ 
							...
							$('#plan').parent().removeClass('active');						
...
function sheepit_load_task_date(){
	if($('.plan_type_sel').length > 0){
		if($('.plan_type_sel').val() == 'P'){
			...
			call_datepicker();	
			// remove time picker
			$('.tsk_time').timepicker({ 'timeFormat': 'h:i A', 'step': 15,'forceRoundTime': true });
			$('.tsk_time').timepicker('remove');
			$('.tsk_time').removeClass('tsk_timepicker');								
		}else{
			// remove date picker
...
function timePicker(){ 
	$('.tsk_timepicker').unbind().timepicker({ 'timeFormat': 'h:i A', 'step': 15,'forceRoundTime': true, 'scrollDefaultTime': '09:30 AM'});
