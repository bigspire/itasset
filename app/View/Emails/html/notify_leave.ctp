    ...
    <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="490" valign="top"  style="padding:0 20px;"><h1 style="font:bold 15px Arial, Helvetica, sans-serif; color:#676767; margin:0 0 10px 0;">Dear <?php echo ucwords($name); ?>,</h1>
          <p style="font:13px Arial, Helvetica, sans-serif; color:#676767; margin:0;">Leave request is <?php echo $status;?> by <?php echo ucfirst($this->Session->read('USER.Login.first_name')).' '.ucfirst($this->Session->read('USER.Login.last_name')); ?>. Please login to My PDCA and check the status of the request.</p><br />

          <p style="font:bold 13px Arial, Helvetica, sans-serif; color:#676767; margin:0;">Below are the leave request details,</p>
          <table width="100%" border="0" cellspacing="2" cellpadding="10" style="border:1px solid #ededed; font:bold 13px Arial, Helvetica, sans-serif; color:#6f6e6e; margin:10px 0 20px 0;">
             ...
             <tr style="background:#f5f4f4;">
              ...
              <td style="color:#2a2a2a;"><?php echo $employee;?></td>
               <td>Leave Type</td>
              <td style="color:#2a2a2a;"><?php echo $leave_type;?></td>         
            </tr>
            <tr style="background:#f5f4f4;">
              <td width="100">Leave From</td>
              <td style="color:#2a2a2a;"><?php echo $this->Functions->format_date($leave_from); echo ' ('.$this->Functions->get_day($leave_from).')';?></td>  			
              <td>No. of Days</td>
              <td style="color:#2a2a2a;"><?php echo $nodays;?></td>
            </tr>
            <tr style="background:#f5f4f4;">
             <td>Leave Till</td>
              <td style="color:#2a2a2a;"><?php echo $this->Functions->format_date($leave_to); echo ' ('.$this->Functions->get_day($leave_to).')';?></td>  
             <td>&nbsp;</td>
              <td style="color:#2a2a2a;">&nbsp;</td>
            
            </tr>
			
		<?php if(!empty($comp_off)):?>
			 <tr style="background:#f5f4f4;">
              <td>Comp. Off For</td>
              <td   colspan="3" style="color:#2a2a2a;">
			  
			  	<?php $date = explode(',', $comp_off);
			...
			<?php if(strtotime($leave_from) < strtotime(date('Y-m-d'))):?>
			 <tr style="background:#f5f4f4;">
              <td colspan="2">Did you inform about the leave already?</td>
              <td  colspan="2" style="color:#2a2a2a;">			  
					Yes
				</td>
