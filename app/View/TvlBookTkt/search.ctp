...
if(!empty($results)): 
	...
	foreach($results as $data):
		...
		$result[] = $this->Functions->match_results($keyword,$data['TvlBookTkt']['tvl_code']);		
		$result[] = $this->Functions->match_results($keyword,$data['TskCustomer']['company_name']);
		$result[] = $this->Functions->match_results($keyword,$data['TvlPlace']['place']);
