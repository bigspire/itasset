...
if(!empty($results)): 
	...
	foreach($results as $data):	
		// split the keywords 
		//$keywords = preg_split("/[\s,]+/", $data['FinExpLimit']['grade_name'] . $data['FinExpLimit']['amount']);		
		$result[] = strtolower($this->Functions->match_results($keyword,$data['HrGrade']['grade_name']));
		$result[] = strtolower($this->Functions->match_results($keyword,$data['FinExpCat']['category']));
